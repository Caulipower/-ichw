...
def calcu(long, a, b):  # 墙的两边长能否被均能表示为若干块砖的长、宽之和
    e = long // a
    f = 0
    while (long != e * a + b * f or (e + f) * a * b > m * n)and e >= 0 and f >= 0:
        e = e - 1
        f = (long - e * a) // b
    if long == e * a + b * f:
        return True
...
def lay_brick(x=0, y=0, i=1):
    ...
    if posi_1[1] == n:   # 判断一种方法是否铺完
        ...
        return
    brick = lay_type(posi_1[0], posi_1[1], 1)  # 横着铺一块砖
    if brick != 0:
        posi_1 = search()  # 符合条件的未铺砖的格子
        lay_brick(posi_1[0], posi_1[1], 1)
        del_lay(bricks[-1][0], bricks[-1][1], bricks[-1][2])
...
def numbers():
    ...
    for i in range(n):
        for j in range(m):
            ...
            t.pendown()
            t.write(number, font=("Arial", 6, "normal"))
            t.penup()
            number += 1
